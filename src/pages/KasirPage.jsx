...
const KasirPage = () => {
  ...
  return (
    <div className="flex flex-col md:flex-row p-2 md:p-4 gap-4 bg-gray-900 min-h-[calc(100vh-64px)] text-white">
      <div className="w-full md:w-1/2 bg-gray-800 p-2 md:p-4 rounded-lg flex flex-col h-fit">
        <h2 className="text-lg md:text-xl font-bold mb-2">Detail Pesanan</h2>
        <input
          type="text"
          placeholder="Nama Customer"
          className="w-full p-2 mb-2 bg-gray-700 rounded text-sm md:text-base"
          value={customerName}
        ...
        <div className="flex-1 overflow-auto max-h-[40vh] md:max-h-[60vh]">
          {cart.map(item => (
            <div 
              key={item.id} 
              className="grid grid-cols-4 md:grid-cols-5 items-center py-2 border-b border-gray-700 text-xs md:text-sm"
            >
              <div>{item.name}</div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center pt-2 gap-2">
          <div className="text-lg md:text-xl">Total: Rp {formatRupiah(total)}</div>
      ...
      <div className="w-full md:w-1/2 overflow-auto">
        <h2 className="text-lg md:text-xl font-bold mb-2 text-center">Menu</h2>
        
        <div className="flex items-center mb-2">
          <hr className="flex-grow border-gray-800" />
          <h3 className="text-base md:text-lg font-bold px-4 text-center">Makanan</h3>
          <hr className="flex-grow border-gray-800" />
        </div>

        <div className="grid grid-cols-3 gap-2 md:gap-3 mb-4">
          {menuItems.makanan.map(item => (
            <div key={item.id} className="bg-gray-800 p-2 md:p-3 rounded-lg text-center hover:bg-gray-700 transition-colors">
              <div className="font-bold text-[10px] md:text-base truncate">{item.name}</div>
              <div className="text-blue-400 text-[10px] md:text-base">Rp {formatRupiah(item.price)}</div>
              <button
                onClick={() => addToCart(item)}
                className="mt-1 md:mt-2 text-blue-400 hover:text-blue-500 w-full"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 md:h-6 md:w-6 mx-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="flex items-center mb-2">
          <hr className="flex-grow border-gray-800" />
          <h3 className="text-base md:text-lg font-bold px-4 text-center">Minuman</h3>
          <hr className="flex-grow border-gray-800" />
        </div>
